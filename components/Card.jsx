        ...
        <div className='space-y-4'>
            ...
            <ul role="list" className="w-full gap-2 space-y-2 ">
                {serviceList.map((service,i) =>
                    <li key={service.html_id} className={`${service.category == 'finance' ?'':'hidden'} w-full md:w-[40vw] divide-y divide-gray-200 rounded-lg bg-white shadow`} onClick={() => setModal(true)}>
            ...
            <ul role="list" className="w-full gap-2 space-y-2 ">
                {serviceList.map((service,i) =>
                    <li key={service.html_id} className={`${service.category == 'manpower'?'':'hidden'} w-full md:w-[40vw] divide-y divide-gray-200 rounded-lg bg-white shadow`} onClick={() => setModal(true)}>
                        <div className="flex w-full items-center justify-between space-x-6 p-6">
                            <div className="flex-1 truncate">
                                <div className="flex items-center space-x-3">
                                    <h3 className="truncate text-sm font-medium text-gray-900">{service.provider_id}</h3>
                                </div>
            ...
            <ul role="list" className="w-full gap-2 space-y-2 ">
                {serviceList.map((service,i) => 
                    <li key={service.html_id} className= {`${service.category == 'infrastructure'?'': 'hidden' }  w-full md:w-[40vw] divide-y divide-gray-200 rounded-lg bg-white shadow`} onClick={() => setModal(true)}>
                        <div className="flex w-full items-center justify-between space-x-6 p-6">
                            <div className="flex-1 truncate">
                                <div className="flex items-center space-x-3">
                                    <h3 className="truncate text-sm font-medium text-gray-900">{service.provider_id}</h3>
